...
export function drawToonFace(ctx: CanvasRenderingContext2D, w: number, h: number, pose: FacePose): void {
  const s = pose.shape;
...
function drawMouth(ctx: CanvasRenderingContext2D, s: Shape, R: number, jawShift: number): void {
  ...
  let gap = jawOpen * R * 0.42 + funnel * R * 0.18 + (upperUp + lowerDown) * R * 0.11;
  gap *= (1 - close * 0.85);
  gap *= (1 - press * 0.9);
